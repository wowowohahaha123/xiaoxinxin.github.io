...
final class Shopisle_Customizer_Upsell {
	...
	public function sections( $manager ) {
		...
		require_once( trailingslashit( get_template_directory() ) . 'inc/customizer/customize-pro/class-shopisle-customize-upsell-main.php' );
		require_once( trailingslashit( get_template_directory() ) . 'inc/customizer/customize-pro/class-shopisle-customize-upsell-text.php' );
		require_once( trailingslashit( get_template_directory() ) . 'inc/customizer/customize-pro/class-shopisle-customize-upsell-section.php' );
		...
		$manager->add_section( new Shopisle_Customizer_Upsell_Main( $manager, 'shopisle-upsell', array(
		...
		$manager->add_section( new Shopisle_Customizer_Upsell_Section( $manager, 'shopisle-upsell-section', array(
		...
		if ( 'page' === get_option( 'show_on_front' ) ) {
			...
			$manager->add_section( new Shopisle_Customizer_Upsell_Text( $manager, 'shopisle-upsell-frontpage-instructions', array(
				'upsell_text' =>
					sprintf( __( 'To customize the Frontpage sections please go to %1$s and select "Your Latest Posts".', 'shop-isle' ),
					sprintf( '<a class="shop_isle_go_to_section" href="%1$s">%2$s</a>', admin_url( 'customize.php?autofocus[control]=show_on_front' ), esc_html__( 'Static Front Page', 'shop-isle' ) ) ),
				'panel'       => 'shop_isle_front_page_sections',
				'priority'    => 1,
			) ) );
		}
